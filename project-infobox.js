...
function projectInfobox(id,a,c){
 ...
 return `<aside class="wiki-facts project-infobox" aria-label="项目简明资料"><div class="infobox-title"><span>项目资料</span><h2>${E(names.chinese||a.title)}</h2><p>${E(names.english||c.p.legal||'')}</p></div>${image}<dl>${rows.map(([k,v])=>`<div class="fact-row"><dt>${E(k)}</dt><dd>${v}</dd></div>`).join('')}</dl><div class="infobox-foot"><a href="#" data-scroll="wiki-sources">资料来源 ↓</a>${exportMiniLinks(id)}<a href="#/graph">产业关系图 →</a></div></aside>`;
}
